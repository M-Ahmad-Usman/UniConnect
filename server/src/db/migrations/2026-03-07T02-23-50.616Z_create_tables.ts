...
export async function up(db: Kysely<any>): Promise<void> {
  await createDepartmentsTable(db, 'departments')
  await createDisciplinesTable(db, 'disciplines')
  await createProgramsTable(db, 'programs')
  await createProgramCurriculaTable(db, 'program_curricula')
  await createUserTypesTable(db, 'user_types')
  await createUsersTable(db, 'users')
  await createUserTypeAssignmentsTable(db, 'user_type_assignments')
  await createStudentsTable(db, 'students')
  await createDesignationsTable(db, 'designations')
  await createTeachersTable(db, 'teachers')
  await createClassesTable(db, 'classes')
  await createSocietiesTable(db, 'societies')
  await createServerTypesTable(db, 'server_types')
  await createServersTable(db, 'servers')
  await createChannelTypesTable(db, 'channel_types')
  await createChannelsTable(db, 'channels')
  await createServerMembershipsTable(db, 'server_memberships')
  await createSocietyMembershipRequestsTable(db, 'society_membership_requests')
  await createCoursesTable(db, 'courses')
  await createCourseAssignmentsTable(db, 'course_assignments')
  await createPostsTable(db, 'posts')
  await createPostAttachmentTypesTable(db, 'post_attachment_types')
  await createPostAttachmentsTable(db, 'post_attachments')
  await createUserRolesTable(db, 'user_roles')
  await createPermissionsTable(db, 'permissions')
  await createUserRolePermissionsTable(db, 'user_role_permissions')
  await createUserRoleAssignmentsTable(db, 'user_role_assignments')
  await createNotificationTypesTable(db, 'notification_types')
  await createNotificationsTable(db, 'notifications')
  await createNotificationPreferencesTable(db, 'notification_preferences')
  await createRefreshTokensTable(db, 'refresh_tokens')
}
...
const createProgramsTable: TableCreationFunction = async (db, tableName) => {
  await db.schema
    ...
    .addCheckConstraint(`chk_${tableName}_semester_positive`, sql<boolean>`total_semesters > 0`)
